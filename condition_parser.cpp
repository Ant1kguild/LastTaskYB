...
shared_ptr<Node> ParseComparison(It &current, It end) {
    ...
    Token &column = *current; /* мои пометки
 * берет значение колонки и проверяет является она датой или событием
 * если это не так выбрасывает исключение */
    ...
    Token &op = *current;/* мои пометки
 * берет значение операции и проверяет является она операцией
 * если это не так выбрасывает исключение */
